...
return: to stop executing, return value is "returned" back to the "caller"
objects: variable,literals,properties,name:value, new Object()
...
arrays: is a special variable, which can hold more than one value, new Array('a', 'b')
array methods:
at(): method returns an indexed element from an array, method returns the same as [], return string
join(): method also joins all array elements into a string, return string
pop(): method removes the last element from an array, array length decrease, return string
...
shift(): removes the first array element and "shifts" all other elements to a lower index, array length decrease, return removed element as string when declared in a variable, return string
...
.length: return count of an array elements, property provides an easy way to append a new element to an array like array[array.length] that returns array, return string
...
delete: leaves undefined holes in the array, use pop() or shift() instead, return array
...
find(): method returns the value of the first array element that passes a test function which is the actual element, should receive callback as parameter, return number
...
reverse(): method reverses the elements in an array, modify existing array, return array
...
forEach(): method calls a function (a callback function) once for each array element to the provided callback function, does not returns a  new array based on the given array, returns “undefined“ or string
...
reduce(): method runs a function on each array element to produce (reduce it to) a single value depends on the second argument or parameter, retain original array
reduceRight(): reduce from right-to-left in the array, retain original array
...
continue: statement breaks one iteration (in the loop), if a specified condition occurs, and continues with the next iteration in the loop, socket_import_stream
